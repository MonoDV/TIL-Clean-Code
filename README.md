...
-   테스트 코드 덕분에 실제 코드의 변경이 쉬워진다
-   지저분한 테스트 코드는 오히려 실제 코드의 변경을 어렵게 만든다
-   테스트 코드를 깨끗하게 만들기 위해 노력하라
...
-   깨끗한 코드 유지하기
    ...
    -   테스트 코드가 복잡할수록 실제 코드를 변경했을 때 테스트를 점점 더 통과하기 어려워진다
    -   이러한 문제들이 발생하지 않도록 깨끗한 단위 테스트 코드를 작성해야 한다
    -   테스트 코드는 실제 코드 못지 않게 중요하다
    -   테스트는 유연성, 유지보수성, 재사용성을 제공한다
        -   테스트가 있으면 변경이 두렵지 않다
        -   테스트 케이스가 없다면 모든 변경사항이 잠정적인 버그다
        -   테스트 커버리지가 높을수록 아키텍처 설계 개선 등에서 공포가 사라진다
        -   즉. 테스트 케이스 덕분에 **실제 코드의 변경**이 쉬워진다
-   깨끗한 테스트 코드
    -   깨끗한 테스트코드를 만들기 위해 필요한 세 가지
        -   가독성
        -   가독성
        -   가독성
    -   BUILD-OPERATE-CHECK 패턴을 적용해 세 부분으로 나눈다
        -   BUILD : 테스트 자료를 만든다
        -   OPERATE : 테스트 자료를 조작하며
        -   CHECK : 조작한 결과가 올바른지 확인한다
    -   위 패턴을 적용하면서 높은 추상화 수준으로 읽기 쉽도록 작성한다
    -   도메인에 특화된 언어로 테스트 코드를 구현하라
        -   시스템 조작 API를 사용하는 대신 API위에다 함수와 유틸리티를 구현한 후 그것을 사용한다
        -   테스트를 짜기도 읽기도 쉬워진다
        -   간결하고 표현력이 풍부한 코드로 리팩토링한다
    -   이중표준
        -   테스트 코드는 실제 코드만큼 효율적일 필요는 없다
        -   약간의 편의를 봐주며 코드를 짜도 무방
-   테스트당 assert 하나
    -   위와 같은 주장은 가혹한 규칙이지만 결론이 하나라서 코드를 이해하기 쉽고 빠른 장점이 있다
    -   `given-when-then` 이라는 관례를 사용
    -   `TEMPLATE METHOD` 패턴을 사용하면 중복 제거 가능
        -   given, when 부분은 부모 클래스에 두고 then 부분은 자식 클래스에 두면 된다
    -   이런저런 방법이 있지만 모두 배보다 배꼽이 더 크다
    -   그냥 assert문을 여럿 사용하는 편이 좋다고 저자가 말했다
        -   `차라리 테스트 함수마다 한 개념만 테스트하라`라는 규칙이 낫다
-   F.I.R.S.T
